...
2. Add and verify your sending domain (e.g. `davkawt.org`). For dev only, you can use the default `onboarding@resend.dev`.
...
- **Cannot find module errors after install**: delete `.next/` and `node_modules/`, reinstall.
...
- **Sign-up email confirmation loop**: in Supabase Auth settings, either disable "Confirm email" for dev or use the link from the Resend log.
